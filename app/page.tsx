...
export default function Home() {
  ...
  const [anilistClickCount, setAnilistClickCount] = useState(0);
  ...
  const handleAnilistClick = () => {
    const newCount = anilistClickCount + 1;
    setAnilistClickCount(newCount);
    if (newCount === 3) {
      window.location.href = "https://anilist.co/user/AndyTropMimi/";
      setAnilistClickCount(0);
    }
  };
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-4 text-xs md:text-sm text-gray-400 font-mono">
              ...
              <div className="space-y-2 border-t border-gray-800 md:border-t-0 md:border-l md:border-gray-800 md:pl-6 pt-4 md:pt-0">
                ...
                <ul className="space-y-2 md:space-y-1 text-[10px] md:text-xs">
                  {/* List item cliquable pour l'Easter egg */}
                  <li onClick={handleAnilistClick} className="cursor-pointer">
                    <span className="text-green-500">✔</span> anilist-updater.sh <span className="text-gray-600 block md:inline">(Auto-tracking anime list via API)</span>
                  </li>
